...
#order       = 5000  # determines size of network:
...
simtime     = 200.0   # simulation time [ms]
...
summary_string = "In Regime: %s (g=%s,eta=%s)" % (scen, g, eta)
...
f.suptitle(summary_string)


n_hist_bins=simtime * 10
n_rasters_max = 50
ax1.set_ylabel('Spiketimes: E_net')
spike_trains = [spiketrain for segment in E_net.get_data().segments for spiketrain in segment.spiketrains ]
for i,sp in enumerate(spike_trains[:n_rasters_max]):
    ax1.plot( sp, i*np.ones_like(sp),'x' )


ax2.set_ylabel('Spiketimes: I_net')
spike_trains = [spiketrain for segment in I_net.get_data().segments for spiketrain in segment.spiketrains ]
for i,sp in enumerate(spike_trains[:n_rasters_max]):
    ax2.plot( sp, i*np.ones_like(sp),'x' )


ax3.set_ylabel('Spiketimes Hist: E_net')
e_spike_trains = [spiketrain for segment in E_net.get_data().segments for spiketrain in segment.spiketrains ]
e_spike_trains = np.hstack(e_spike_trains)
ax3.hist(e_spike_trains, bins=n_hist_bins)

ax4.set_ylabel('Spiketimes Hist: I_net')
i_spike_trains = [spiketrain for segment in I_net.get_data().segments for spiketrain in segment.spiketrains ]
i_spike_trains = np.hstack(i_spike_trains)
ax4.hist(i_spike_trains, bins=n_hist_bins)

if not os.path.exists("_output"):
    os.makedirs('_output')
pylab.savefig("_output/brunnel_%s.svg" % summary_string)
